...
class Grid
  def initialize(live_cells)
    ...
    @width = (live_cells.map(&:x).max || 0) + 1
    @height = (live_cells.map(&:y).max || 0) + 1
  end
...
square = [LiveCell.new(1,1), LiveCell.new(1,2), LiveCell.new(2,1), LiveCell.new(2,2)]
random_cells = (1..400).map do 
  LiveCell.new(rand(40), rand(40))
end.uniq

g = Grid.new(random_cells)
100.times do
  system('clear')
  puts g.to_s
  g = g.tick
